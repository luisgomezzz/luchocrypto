...
def estrategia_divergencias (symbol,timeframe,limit,sma_length,sma_macd_length,umbral,largo):
    ...
    ###########################################################################################################################
    data['trade'] = np.where(util.crossover_dataframe(data.Indicator2, data.Indicator1) & (data.adx >=umbral),-1,np.where(util.crossover_dataframe(data.Indicator1, data.Indicator2) & (data.adx >=umbral),-2,-1.5))
